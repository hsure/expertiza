module SubmittedContentHelper
  ...
  def list_sub_directories (file, participant)
    ...
    Dir.foreach( file ) {|path|
      ...
      ret += "<input type=hidden id='filenames_#{index}' name='filenames[#{index}]' value='"+File.dirname(display)+"/" +File.basename(path)+"'>"
      if File.ftype( display ) == "directory"
        ...
        ret += link_to path, :controller=>'submitted_content', :action => 'edit', :id => participant.id, :download => File.basename(path), "current_folder[name]" =>  File.dirname(display)
        ret += "</li>"
        ret += list_sub_directories(display, participant)
      else
        ret += link_to path, :controller=>'submitted_content', :action => 'edit', :id => participant.id, :download => File.basename(path), "current_folder[name]" =>  File.dirname(display)
        ret += "</li>"
      end
  ...
  def self.unzip_file(file_name, unzip_dir, should_delete)
    ...
    Zip::ZipFile::open(file_name) {
        ...
        file_path = File.join(unzip_dir, FileHelper::sanitize_filename(e.name))
